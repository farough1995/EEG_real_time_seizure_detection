...
for seed_num in args.seed_list:
    ...
    if args.checkpoint:
        if args.last:
            ckpt_path = (
                ...
                + "/"
                + args.project_name
                + "/ckpts/best_{}.pth".format(str(seed_num))
            )
        elif args.best:
            ckpt_path = (
                args.dir_result
    ...
    if args.optim == "adam":
        optimizer = optim.Adam(
            model.parameters(), lr=args.lr_init, weight_decay=args.weight_decay
        )
    elif args.optim == "sgd":
        optimizer = optim.SGD(
            model.parameters(),
    ...
    if args.lr_scheduler == "CosineAnnealing":
        scheduler = CosineAnnealingWarmUpRestarts(
            optimizer,
            T_0=args.t_0 * one_epoch_iter_num,
            T_mult=args.t_mult,
            eta_max=args.lr_max,
            T_up=args.t_up * one_epoch_iter_num,
            gamma=args.gamma,
        )
    ...
    if args.last:
        ckpt_path = (
            ...
            + "/"
            + args.project_name
            + "/ckpts/last_{}.pth".format(str(seed_num))
        )
    elif args.best:
        ckpt_path = (
            args.dir_result
...
for result, tpr, tnr in list_of_test_results_per_seed:
    os.system(
        "echo  'seed_case:{} -- auc: {}, apr: {}, f1_score: {}, tpr: {}, tnr: {}'".format(
            str(result[0]),
            str(result[1]),
            str(result[2]),
            str(result[3]),
    ...
    tnr_list.append(tnr)
os.system(
    "echo  'Total average -- auc: {}, apr: {}, f1_score: {}, tnr: {}, tpr: {}'".format(
        str(np.mean(auc_list)),
        str(np.mean(apr_list)),
        str(np.mean(f1_list)),
        str(np.mean(tpr_list)),
        str(np.mean(tnr_list)),
    )
)
os.system(
    "echo  'Total std -- auc: {}, apr: {}, f1_score: {}, tnr: {}, tpr: {}'".format(
        str(np.std(auc_list)),
        str(np.std(apr_list)),
        str(np.std(f1_list)),
        str(np.std(tpr_list)),
