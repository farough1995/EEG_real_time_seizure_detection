...
parser.add_argument(
    "--lr-scheduler", type=str, default="Single", choices=["CosineAnnealing", "Single"]
)
parser.add_argument(
    "--lr_init", type=float, default=1e-3
)  # not being used for CosineAnnealingWarmUpRestarts...
parser.add_argument("--lr_max", type=float, default=4e-3)
parser.add_argument(
    "--t_0", "-tz", type=int, default=5, help="T_0 of cosine annealing scheduler"
)
parser.add_argument(
    "--t_mult", "-tm", type=int, default=2, help="T_mult of cosine annealing scheduler"
)
parser.add_argument(
    "--t_up",
    "-tup",
...
parser.add_argument(
    "--momentum", "-mo", type=float, default=0.9, help="Momentum of optimizer"
)
parser.add_argument(
    "--weight_decay", "-wd", type=float, default=1e-6, help="Weight decay of optimizer"
)
parser.add_argument(
    "--loss-decision",
    type=str,
    default="max_division",
    choices=["mean", "max_division"],
)
...
parser.add_argument(
    "--requirement-target", type=int, default=None, help="unit is second"
)
...
parser.add_argument(
    "--hyperopt-model-name", type=str, default="xgboost_classification"
)  # model name
...
parser.add_argument(
    "--sincnet-input-normalize",
    type=str,
    default="none",
    choices=["none", "layernorm", "batchnorm"],
)
parser.add_argument(
    "--sincnet-layer-num", type=int, default=1, help="select int between 1 ~ 3"
)
...
parser.add_argument(
    ...
    type=str,
    default="standard",
    choices=["standard", "inverted_bottleneck"],
)
parser.add_argument(
    "--block-reinforcement-methods", type=str, default=None, choices=["cbam", None]
)
parser.add_argument(
    "--block-temporal-methods",
    type=str,
    default="stand_alone",
    choices=["time", "stand_alone"],
)
...
parser.add_argument(
    ...
    type=str,
    default="bipolar",
    choices=["unipolar", "bipolar", "uni_bipolar"],
)
parser.add_argument(
    "--task-type",
    "-tt",
...
parser.add_argument(
    "--binary-sampler-type", type=str, default="6types", choices=["6types", "30types"]
)
...
# parser.add_argument("--disease_type", type=str, default="bckg seiz")
