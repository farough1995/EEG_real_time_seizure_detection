...
class BasicBlock(nn.Module):
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(
                planes, planes, kernel_size=(1, 9), stride=1, padding=(0, 4), bias=False
            ),
            nn.BatchNorm2d(planes),
        )
...
class RESNET_DILATION_LSTM(nn.Module):
    def __init__(self, args, device):
            ...
            self.feat_models = nn.ModuleDict(
                [
                    ...
                    [
                        "sincnet",
                        SINCNET_FEATURE(
                            args=args, num_eeg_channel=self.num_data_channel
                        ),  # padding to 0 or (kernel_size-1)//2
                    ],
                ]
            )
        ...
        self.dilated_conv2d_1 = conv2d_bn(
            self.num_data_channel, 32, (1, 51), (1, 4), (0, 25), (1, 1)
        )
        self.dilated_conv2d_2 = conv2d_bn(
            self.num_data_channel, 16, (1, 51), (1, 4), (0, 50), (1, 2)
        )
        self.dilated_conv2d_3 = conv2d_bn(
            self.num_data_channel, 16, (1, 51), (1, 4), (0, 100), (1, 4)
        )
    ...
    def init_state(self, device):
        self.hidden = (
            torch.zeros(self.num_layers, self.args.batch_size, self.hidden_dim).to(
                device
            ),
            torch.zeros(self.num_layers, self.args.batch_size, self.hidden_dim).to(
                device
