...
class InvertedResidual(nn.Module):
    def __init__(
        self, inp, hidden_dim, oup, kernel_size, stride, use_se, use_hs, is_psd
    ):
    ...
    def __init__(self, args, device):
        ...
        if args.enc_model == "psd1" or args.enc_model == "psd2":
            ...
            self.conv1 = conv_3x3_bn(
                self.num_data_channel, input_channel, 2, self.is_psd
            )
        else:
        ...
        output_channel = (
            _make_divisible(output_channel[mode] * self.width_mult, 8)
        ...
        if self.args.enc_model != "raw":
            ...
            if self.args.enc_model == "sincnet":
                x = x.reshape(
                    x.shape[0],
                    1,
                    self.args.num_channel * self.args.sincnet_bandnum,
                    x.shape[-1],
                )
        else:
