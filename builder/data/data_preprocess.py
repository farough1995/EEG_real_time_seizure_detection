...
class Detector_Dataset(torch.utils.data.Dataset):
    def __init__(self, args, data_pkls, augment, data_type="training dataset"):
        ...
        patient_dev_dict = {}
        for idx, pkl in enumerate(
            tqdm(data_pkls, desc="Loading edf files of {}".format(data_type))
        ):
            ...
            if "training dataset" != data_type:
                ...
                if (type1 == "0") and (
                    patient_dev_dict[pat_id][0] >= args.dev_bckg_num
                ):
                    continue
                if (type1 != "0") and (
                    patient_dev_dict[pat_id][2] >= args.dev_bckg_num
                ):
                    continue
                if type1 == "0":
        ...
        for idx, seizure in enumerate(args.seiz_classes):
            ...
            print(
                "Number of {} slices: total:{} - beg:{}, middle:{}, end:{}, whole:{}".format(
                    seizure,
                    str(total_seiz_num),
                    str(beg_slice_num),
                    str(middle_slice_num),
                    str(end_slice_num),
                    str(whole_slice_num),
                )
            )
        ...
        if "training dataset" != data_type:
            ...
            for pat_info in patient_dev_dict:
                ...
                print(
                    "(Non-)Patient: {} has normals:{}, seizures:{}, mid_seizures:{}".format(
            ...
            print(
                "Total normals:{}, seizures with boundaries:{}, seizures with middles:{}".format(
                    str(num_normals),
                    str(num_seizures_boundaries),
                    str(num_seizures_middles),
                )
            )
...
def get_data_preprocessed(args, mode="train"):
    ...
    eval_data_path = args.data_path + "/dataset-tuh_task-binary_datatype-eval_v6"
    ...
    eval_dir = search_walk({"path": eval_data_path, "extension": ".pkl"})
    # print(train_dir)
    # print(dev_dir)
    random.shuffle(train_dir)
    random.shuffle(dev_dir)
    random.shuffle(eval_dir)
    ...
    # 
    aug_val = ["0"] * len(dev_dir)
    aug_test = ["0"] * len(eval_dir)

    train_data = Detector_Dataset(
        args, data_pkls=train_dir, augment=aug_train, data_type="training dataset"
    )
    ...
    val_data = Detector_Dataset(
        args, data_pkls=dev_dir, augment=aug_val, data_type="validation dataset"
    )
    test_data = Detector_Dataset(
        args, data_pkls=eval_dir, augment=aug_test, data_type="test dataset"
    )
    ...
    train_loader = DataLoader(
        ...
        pin_memory=True,
        sampler=sampler,
        collate_fn=eeg_binary_collate_fn,
    )
    ...
    return (
        ...
        val_loader,
        test_loader,
        len(train_data._data_list),
        len(val_data._data_list),
        len(test_data._data_list),
    )
